...
public class tableroghost extends javax.swing.JPanel{
    ...
    public tableroghost(){
        ...
        while(jj1 == false && jj2 == false){
            ...
            if(login.listaj[i] != null){
                if(login.listaj[i].usuario.equalsIgnoreCase(menu.usuario2)){
        ...
        while(jj1 == false || jj2 == false){
            for(int i = 0; i < login.listaj.length; i++){
                if(login.listaj[i] == null && jj1 == false){
                   jugador1 = new jugadores(login.usuario,login.contraseña,0);
                   login.listaj[i] = jugador1; 
                   jj1 = true;
                }
                if(login.listaj[i] == null && jj2 == false){
                  jugador2 = new jugadores(menu.usuario2,menu.contraseña2,0);
                  login.listaj[i] = jugador2;
                  jj2 = true;
                }
            }
        } 
        if(menu.dificultad == 'a'){
                fls = 2;
                cls = 5;
                cls2 = 1;
                flsj2 = 4;
    ...
    amconta = 0;
    }else if(menu.modoj == 'b'){
        amconta = 0;
        //Jugador2 
            for(int f = 0; f < 3; f++){
                for(int c = 1; c < 5; c++){
                    if(menu.cordesp[f][c] == true){
                        lugares[f][c].setIcon(j2);
                        if(amconta == 0){
                            espacios[f][c] = false;
                            amconta +=1;
